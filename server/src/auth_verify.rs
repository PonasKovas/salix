...
pub struct AuthVerifyLayer {
	pub db: PgPool,
}

impl<S> Layer<S> for AuthVerifyLayer {
	type Service = AuthVerifyService<S>;
...
#[derive(Debug, Clone)]
pub struct AuthVerifyService<S> {
	inner: S,
	db: PgPool,
}

impl<S, ReqBody, ResBody> Service<http::Request<ReqBody>> for AuthVerifyService<S>
where
...
impl<S: NamedService> NamedService for AuthVerifyService<S> {
